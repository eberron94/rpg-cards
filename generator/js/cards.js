...
function card_has_all_tags(card, tags) {
  var test = true;
  for (var i in tags) {
    test &= card_has_tag(card, tags[i]);
  }
  return test;
}

function card_has_any_tags(card, tags) {
  var test = false;
  for (var i in tags) {
    test |= card_has_tag(card, tags[i]);
  }
...
function card_complex_filter(card, tags) {
  var req = 0;
  var alw = 0;
  for (var i in tags) {
    var tag = "" + tags[i];
    if (tag.startsWith("!")) {
      //Exclude this Tag
      if (card_has_tag(card, tag.replace("!", ""))) {
        return false;
      }
    } else if (tag.startsWith("&")) {
      //Require this Tag
      req += card_has_tag(card, tag.replace("&", "")) ? 1 : -100000;
    } else if (tag.startsWith("|")) {
      //Allow this Tag
      alw += card_has_tag(card, tag.replace("|", "")) ? 100000 : -1;
    } else {
      //Allow this tag
      alw += card_has_tag(card, tag) ? 100000 : -1;
    }
  }
  return req >= 0 && alw >= 0;
}
...
function card_data_color_back(card_data, options) {
  return (
    card_data.color_back || card_data.color || options.default_color || "white"
  );
}
...
function card_element_subtitle(params, card_data, options) {
  ...
  return (
    '<div class="card-element card-subtitle" style="font-size:' +
    subtitle_text_font +
    'pt">' +
    subtitle +
    "</div>"
...
function card_element_center(params, card_data, options) {
  var result = "";
  result +=
    '<div class="card-element card-description-line" style="text-align: center">';
  result += '   <p class="card-p card-description-text">' + params[0] + "</p>";
  result += "</div>";
  return result;
}
...
function card_element_justify(params, card_data, options) {
  var result = "";
  result +=
    '<div class="card-element card-description-line" style="text-align: justify; hyphens: auto">';
  result += '   <p class="card-p card-description-text">' + params[0] + "</p>";
  result += "</div>";
  return result;
}
...
function card_element_section(params, card_data, options) {
  ...
  if (params[1]) {
    section += '<span style="float:right">' + params[1] + "</span>";
  }
  return (
    '<h3 class="card-section" style="color:' +
    color +
    "; font-size:" +
    section_text_font +
    'pt">' +
...
function card_element_table_header(params, card_data, options) {
  var result = "";
  result += '<tr class="card-element card-table-header">';
  result += params
    .map(param => '   <th class="card-table-cell">' + param + "</th>")
    .join("\n");
  result += "</tr>";
  return result;
}
...
function card_element_table_row(params, card_data, options) {
  var result = "";
  result += '<tr class="card-element card-table-row">';
  result += params
    .map(param => '   <td class="card-table-cell">' + param + "</td>")
    .join("\n");
  result += "</tr>";
  return result;
}
...
var card_element_generators = {
  ...
  tablehead: card_element_table_header,
  tableheader: card_element_table_header,
  table: card_element_table_row
};
...
function card_generate_contents(contents, card_data, options) {
  ...
  var contentGroup = split_content([...contents, ""]);
    ...
    .map(value => {
      if (Array.isArray(value))
        return (
          ...
          value.map(buildCard).join("\n") +
          "</table>"
        );
      else return buildCard(value);
    })
    .join("\n");
  result += "</div>";
  return result;
...
function split_content(contents, split = []) {
  if (contents.length === 0) return split;

  let queue = [];
  contents.forEach(value => {
    if (
      ["table", "tableheader", "tablehead"].includes(
        card_data_split_params(value)[0]
      )
    ) {
      queue.push(value);
    } else {
...
function card_generate_color_style(color, options) {
  ...
  //TODO
  return (
    "color:" + color + "; border-color:" + color + "; background-color:" + color
  );
}
...
function card_pages_add_padding(cards, options) {
  ...
  if (last_page_cards !== 0) {
    return cards.concat(
      card_generate_empty(cards_per_page - last_page_cards, options)
    );
  } else {
    return cards;
  }
...
function card_pages_wrap(pages, options) {
  ...
  for (var i = 0; i < pages.length; ++i) {
    ...
    result +=
      '<page class="page page-preview" size="' + size + '" ' + style + ">\n";
    result += pages[i].join("\n");
    result += "</page>\n";
  }
  return result;
